            ...
            <div className="rounded-3xl border bg-card p-6 sm:p-8 shadow-soft h-fit">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  ...
                  <div className="font-mono text-lg font-semibold">{id}</div>
                </div>
                <div className="rounded-full bg-primary/10 px-3 py-1 text-sm font-semibold text-primary">{STAGES[stage].label}</div>
              </div>
            ...
            {order && (
              <aside className="rounded-3xl border bg-card p-6 shadow-soft h-fit space-y-6">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Placed on {new Date(order.date).toLocaleString()}</p>
                </div>
                
                <div className="space-y-3 border-t pt-4 text-sm">
                  <div className="flex justify-between items-start">
                    <span className="text-muted-foreground">Location</span>
                    <span className="font-semibold text-right max-w-[200px]">{order.branch}</span>
                  </div>
                ...
                <div className="border-t pt-4 flex justify-between font-display text-lg font-bold">
                  ...
                  <span className="text-primary">{order.total} DKK</span>
                </div>
              </aside>
            )}
          </div>
