...
import { Loader2, Check, Store, CalendarOff, Repeat, Plus, Trash2, AlertTriangle, ArrowRight } from "lucide-react";
import {
...
function ColorCell({ color }: { color?: string | null }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="inline-block h-5 w-5 shrink-0 rounded-full border shadow-sm" style={{ backgroundColor: hex }} />
...
function OrderStatusesPanel() {
  ...
  const handleCreateTransition = async () => {
    if (!transitionForm.fromStatusId || !transitionForm.toStatusId) { toast.error("Select both statuses."); return; }
    setSubmitting(true);
    try {
  ...
  const statusColumns = useMemo<ColumnDef<OrderStatusDto>[]>(() => [
    { id: "name",        header: "Name EN",  accessorKey: "name",        cell: ({ row }) => <span className="font-medium">{row.original.name}</span> },
    { id: "nameDa",      header: "Name DA",  accessorKey: "nameDa",      cell: ({ row }) => <span className="text-muted-foreground">{row.original.nameDa ?? "—"}</span> },
  ...
  const transitionColumns = useMemo<ColumnDef<OrderStatusTransitionDto>[]>(() => [
    { id: "fromStatusName", header: "From",    accessorKey: "fromStatusName", cell: ({ row }) => <span className="font-medium">{row.original.fromStatusName}</span> },
    { id: "__arrow",        header: "",        enableSorting: false, enableColumnFilter: false, size: 32, cell: () => <ArrowRight className="h-3.5 w-3.5 text-muted-foreground" /> },
    { id: "toStatusName",   header: "To",      accessorKey: "toStatusName",   cell: ({ row }) => <span className="font-medium">{row.original.toStatusName}</span> },
    { id: "serviceType",    header: "Service", accessorKey: "serviceType",    cell: ({ row }) => <ServiceBadge type={row.original.serviceType} /> },
    { id: "__actions",      header: "Actions", enableSorting: false, enableColumnFilter: false, cell: ({ row }) => <ActionButtons onDelete={() => handleDeleteTransition(row.original.id)} /> },
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ], []);
    ...
    <div className="space-y-4">
      <div className="inline-flex items-center rounded-lg bg-muted p-1 gap-0.5">
        <button data-tagid="button-order-statuses-statuses-tab"
          onClick={() => setInnerTab("statuses")}
          className={`rounded-md px-3 py-1.5 text-xs font-medium transition-all ${innerTab === "statuses" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
        >
          Statuses ({statuses.length})
        </button>
        <button data-tagid="button-order-statuses-transitions-tab"
          onClick={() => setInnerTab("transitions")}
          className={`rounded-md px-3 py-1.5 text-xs font-medium transition-all ${innerTab === "transitions" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
        >
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input data-tagid="input-order-statuses-name-en" value={statusForm.name}
                  onChange={e => setStatusForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. New" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Name (DA)</Label>
                <Input data-tagid="input-order-statuses-name-da" value={statusForm.nameDa ?? ""}
                  onChange={e => setStatusForm(f => ({ ...f, nameDa: e.target.value }))} placeholder="e.g. Ny" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Input data-tagid="input-order-statuses-display-order" type="number" min={1}
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-3">
                <input data-tagid="input-order-statuses-color" type="color"
                ...
                <Input data-tagid="input-order-statuses-color-hex"
                  value={statusForm.color ?? ""} placeholder="#3B82F6" className="font-mono"
                  onChange={e => setStatusForm(f => ({ ...f, color: e.target.value }))} />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <Select value={String(transitionForm.fromStatusId)} onValueChange={v => setTransitionForm(f => ({ ...f, fromStatusId: Number(v) }))}>
                ...
                <SelectContent>
                  {activeStatuses.map(s => <SelectItem key={s.id} value={String(s.id)}>{s.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              ...
              <Select value={String(transitionForm.toStatusId)} onValueChange={v => setTransitionForm(f => ({ ...f, toStatusId: Number(v) }))}>
                <SelectTrigger><SelectValue placeholder="Select…" /></SelectTrigger>
                <SelectContent>
                  {activeStatuses.filter(s => s.id !== transitionForm.fromStatusId).map(s => <SelectItem key={s.id} value={String(s.id)}>{s.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
...
// ── Reservation Statuses Panel ────────────────────────────────────────────────

const RESERVATION_STATUSES_INFO = [
  { name: "Pending",   color: "#F59E0B", desc: "Awaiting confirmation from the restaurant" },
  { name: "Confirmed", color: "#10B981", desc: "Accepted and confirmed by the restaurant" },
  { name: "Cancelled", color: "#EF4444", desc: "Reservation was cancelled" },
];

function ReservationStatusesPanel() {
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Reservation statuses are system-defined and cannot be added or removed.
      </p>
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="px-5 py-3 text-left font-semibold text-gray-700">Color</th>
              <th className="px-5 py-3 text-left font-semibold text-gray-700">Description</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {RESERVATION_STATUSES_INFO.map(s => (
              <tr key={s.name}>
                <td className="px-5 py-3 font-medium text-gray-800">{s.name}</td>
                <td className="px-5 py-3"><ColorCell color={s.color} /></td>
                <td className="px-5 py-3 text-gray-500">{s.desc}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
type SettingsTab = "schedule" | "pricing" | "closures" | "orderStatuses" | "reservationStatuses";
...
const TABS: { id: SettingsTab; label: string }[] = [
  ...
  { id: "reservationStatuses", label: "Reservation Statuses" },
];
...
function ServiceStatusPage() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {activeTab === "orderStatuses" && <OrderStatusesPanel />}
      {activeTab === "reservationStatuses" && <ReservationStatusesPanel />}
    </div>
  );
}
