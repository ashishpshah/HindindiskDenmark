    ...
    >
      <div className="mx-auto flex h-24 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6">
        ...
        <div className="flex items-center gap-1 lg:hidden">
          ...
          <button className="rounded-full p-2 hover:bg-accent" onClick={() => setOpen(true)} aria-label="Open menu" aria-expanded={open} aria-controls="mobile-nav">
        ...
        {open && (
          ...
          >
            ...
            <nav className="flex flex-col gap-1 px-6 py-6 text-xl">
              {navLinks.map((l) => (
                <Link key={l.to} to={l.to} onClick={() => setOpen(false)} className="border-b border-white/10 py-3 hover:text-primary">
                  {l.label}
                </Link>
              ))}
